...
class QPTA(object):
    ...
    def __init__(self):
        ...
        self.rate = rospy.Rate(10) # time step of 0.2 s^1 = 5 Hz
        ...
        self.pub_stern_angles             = rospy.Publisher('QP/thrusterAllocation/pod_angle_input', podAngle, queue_size=1)
        self.pub_stern_thruster_setpoints = rospy.Publisher("QP/thrusterAllocation/stern_thruster_setpoints", SternThrusterSetpoints, queue_size=1)
        self.pub_bow_control              = rospy.Publisher("QP/bow_control", bowControl, queue_size=1)
    ...
    def solve_QP(self,tau_d):
        ...
        while not solution.success # AND TIMECONSTRAINT IS OK
            s_bnd += 1.0 # TODO set this to whatever suitable
            bnds = ((-25,25),(-25,25),(-6.1,14),\
                    (-2*np.pi,2*np.pi),(-2*np.pi,2*np.pi),\
                    (-s_bnd,s_bnd),(-s_bnd,s_bnd),(-s_bnd,s_bnd)) 
            
            # TODO is it possible to continue the optimization using the slack variables from where they ended the last time?
            # x = solution.x
            # s1, s2, s3 = x[-3:]
