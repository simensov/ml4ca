...
rpm_max = {'bow': 100, 'stern': 100}
diameters = {'bow':0.2, 'stern': 0.3}

def power(n,which):
    return np.abs(n * rpm_max[which])**3 * diameters[which]**5

powers = [[[],[],[]], [[],[],[]], [[],[],[]]] # Contains power timeseries on [bow,port,star] for each method
for i in range(len(methods)):
